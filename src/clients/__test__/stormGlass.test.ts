...
describe('StormGlass client', () => {
  ...
  it('should return the normalized forecast from the StormGlass service', async () => {
    ...
    mockedAxios.get.mockResolvedValue({ data: stormGlassWeather3HoursFicture });

    const stormTGlass = new StormGlass(mockedAxios);
  ...
  it('should get a generic error from StormGlass service when the request fails before reaching the service', async () => {
    ...
    await expect(stormGlass.fetchPoints(lat, lng)).rejects.toThrowError(
